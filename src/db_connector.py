...
def save_users_visit(user_id):
    get_visits_connection().cursor().execute(
        (
            "INSERT INTO "
            f"'{user_visits_table_name}' AS src (user_id, last_visit, counter) "
            f'VALUES (?, "{datetime.now().isoformat()}",  1) '
            "ON CONFLICT (user_id) DO UPDATE "
            f'SET last_visit="{datetime.now().isoformat()}", counter=excluded.counter+1;'
        ),
        (user_id,),
    )
